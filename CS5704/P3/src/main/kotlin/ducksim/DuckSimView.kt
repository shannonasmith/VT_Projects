...
/**
 * Represents the view component of the DuckSim application, responsible for rendering the simulation environment.
 *
 * @property duckPond The DuckPond containing ducks to be displayed in the view.
 */
...
class DuckSimView(private val duckPond: DuckPond) : JPanel() {
    ...
    val flyTimer = Timer(15) { // sorry they fly to darn slow! WAS 25
        animationX += 5
        repaint()
    }

    //// Fields for quacking animation ////
    val quackTimer: Timer = Timer(80) { repaint() } // changed from 100 to 80
    private var quackCounter = 0

    /**
     * Initializes the DuckSimView by setting up key bindings and actions.
     */
    init {
    ...
    /**
     * Sets the escape action to be performed when the escape key is pressed.
     *
     * @param action The action to perform when the escape key is pressed.
     */
    ...
    private fun paintQuack(g: Graphics) {
        ...
        g.color = duckPond.currentDuck!!.color // Color.BLACK
    ...
    private fun paintDuck(g: Graphics, duck: Duck, rect: Rectangle) {
        ...
        g.color = if (isActive(duck)) duck.color else Color.WHITE // Color.CYAN else Color.WHITE
        fillRect(g, rect)
        g.color = if (!isActive(duck)) Color.GRAY else Color.DARK_GRAY  // Color.GRAY
        ...
        g.color = if (!duckPond.selectedDucks.contains(duck)) Color.WHITE else duck.color
        fillRect(g, reduceRect(rect, 20))
        ...
        //// Print "W" if duck is in welcoming committee ////
        if (duck.isOnDSWC) {
    ...
    private fun drawCenteredTextInRectangle(
        ...
        //// centered (positive offset move it right) ////
        val xPos = rectangle.x + rectangle.width / 2 - textWidth / 2 + xOffset
        //// top (positive offset move it down) ////
        val yPos = rectangle.y + yOffset
