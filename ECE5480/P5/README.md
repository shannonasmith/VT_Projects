# ECE 5480 – Project 5: Cross-Site Scripting (XSS) Attack Lab
...
Elgg has built-in XSS countermeasures; this lab environment has them intentionally disabled
to demonstrate how the underlying vulnerability works.
...
(Samy's). Any user who views the profile executes the script, which simply confirms the
injection point is exploitable:

```html
<script>alert('XSS');</script>
```

When another user (Alice) viewed Samy's profile, the alert box fired immediately — proving
the profile field renders injected script rather than treating it as inert text.
...
Displaying the cookie to the victim is not useful to an attacker — the goal is to get that
value sent *back* to the attacker. This payload uses `document.write()` to inject a hidden
1x1 image tag whose `src` points to the attacker's own listening server, with the victim's
cookie appended as a URL parameter:
...
JavaScript security tokens (which are already valid in the victim's session) to add the
attacker as a friend automatically, with no user interaction beyond viewing the profile:
